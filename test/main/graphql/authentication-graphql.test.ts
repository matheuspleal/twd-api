...
describe('AuthenticationGraphQL', () => {
  ...
  describe('Sign Up', async () => {
    beforeAll(() => {
      signUpMutation = `
        mutation SignUp($fullName: String!, $birthdate: Date!, $email: String!, $password: String!) {
          signUp(fullName: $fullName, birthdate: $birthdate, email: $email, password: $password) {
            user {
              id
              fullName
              birthdate
    ...
    test('sign up with invalid email', async () => {
      const { fullName, birthdate, password } = makeRequiredSignUpInputStub()
    ...
    test('sign up with invalid password', async () => {
      const { fullName, birthdate, email } = makeRequiredSignUpInputStub()
    ...
    test('sign up with invalid birthdate', async () => {
      const { fullName, email, password } = makeRequiredSignUpInputStub()
    ...
    test('sign up successfully', async () => {
      const { fullName, birthdate, email, password } =
        makeRequiredSignUpInputStub()
      ...
      const { statusCode, body } = await request(app.server)
        ...
        .send({
          query: signUpMutation,
          variables: { fullName, email, password, birthdate },
        })

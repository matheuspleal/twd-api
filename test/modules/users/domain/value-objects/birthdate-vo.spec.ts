...
describe('BirthdateVO', () => {
  ...
  it('should be able to returns get current age in years', () => {
    const mockDate = new Date('2024-01-01T00:00:00.000Z')
    vi.setSystemTime(mockDate)

    const fakeValidBirthdate = '2000-01-01'
    sut = new BirthdateVO({ value: fakeValidBirthdate })

    const isValid = sut.isValid()

    const currentAgeInYears = sut.getCurrentAgeInYears()

    expect(isValid).toBe(true)
    expect(sut.toValue()).toBeInstanceOf(Date)
    expect(sut.toString()).toMatch(ISODateRegExp)
    expect(currentAgeInYears).toBeGreaterThanOrEqual(24)
